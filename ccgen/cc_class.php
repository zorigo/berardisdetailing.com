   ...
   */
     ...
     */    
     public function createCampaignXML( $id, $params = array() ) {  
            ...
            $title_node = $xml_object->addChild("title", htmlentities($params['cmp_name']));
            ...
            $name_node = $campaign_node->addChild("Name", urldecode(htmlentities($params['cmp_name'])));
            ...
            $subj_node = $campaign_node->addChild("Subject", urldecode(htmlentities($params['cmp_subject'])));
            $from_name_node = $campaign_node->addChild("FromName", urldecode(htmlentities($params['cmp_from_name'])));
            ...
            $as_weblink_node = $campaign_node->addChild("ViewAsWebpageLinkText", urldecode(htmlentities($as_web_lnk_txt)));
            #$as_web_txt = ($view_as_webpage == 'YES') ? ($params['cmp_as_webtxt']) : ('');
            $as_web_txt = $params['cmp_as_webtxt'];
            $as_webtxt_node = $campaign_node->addChild("ViewAsWebpageText", urldecode(htmlentities($as_web_txt)));
            ...
            $text_reminder_node = $campaign_node->addChild("PermissionReminderText", urldecode(htmlentities($permission_reminder_text)));
            $grt_sal_node = $campaign_node->addChild("GreetingSalutation", htmlentities($params['cmp_grt_sal']));
            $grt_name_node = $campaign_node->addChild("GreetingName", htmlentities($params['cmp_grt_name']));
            $grt_str_node = $campaign_node->addChild("GreetingString", htmlentities($params['cmp_grt_str']));
            $org_name_node = $campaign_node->addChild("OrganizationName", htmlentities($params['cmp_org_name']));
            $org_addr1_node = $campaign_node->addChild("OrganizationAddress1", htmlentities($params['cmp_org_addr1']));
            $org_addr2_node = $campaign_node->addChild("OrganizationAddress2", htmlentities($params['cmp_org_addr2']));
            $org_addr3_node = $campaign_node->addChild("OrganizationAddress3", htmlentities($params['cmp_org_addr3']));
            $org_city_node = $campaign_node->addChild("OrganizationCity", htmlentities($params['cmp_org_city']));
            ...
            switch($params['org_country']){
                ...
                break;
                default:
                $international_state = $params['org_state'];
            }
            ...
            $org_zip_node = $campaign_node->addChild("OrganizationPostalCode", htmlentities($params['org_zip']));
            ...
            $fwd_email_node = $campaign_node->addChild("ForwardEmailLinkText", htmlentities($fwd_txt));
            ...
            $sub_link_node = $campaign_node->addChild("SubscribeLinkText", htmlentities($sub_txt));
            $email_format_node = $campaign_node->addChild("EmailContentFormat", $params['cmp_mail_type']);      
            if($params['cmp_type'] != 'STOCK'){
            $html_body_node = $campaign_node->addChild("EmailContent", htmlentities($params['cmp_html_body']));     
            $text_body_node = $campaign_node->addChild("EmailTextContent", "<Text>".strip_tags($params['cmp_text_body'])."</Text>");  
            $campaign_style_sheet = ($params['cmp_mail_type'] == 'XHTML') ? ($params['cmp_style_sheet']) : (''); 
            $style_sheet_node = $campaign_node->addChild("StyleSheet", $campaign_style_sheet);         
            }
